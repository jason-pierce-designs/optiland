...
import { getLocalMetadata, getImgUrlForCollection } from "../lib/helpers";
...
export default function NFTCard({ id, collection }: NFTCardProps) {
  const [metadata, setMetadata] = useState<BunnyMetadata>();
  const [image] = useState<string>(getImgUrlForCollection(collection, id));

  useEffect(() => {
    if (!metadata) {
      getLocalMetadata(collection, id).then((data) =>
        setMetadata(data as BunnyMetadata)
      );
    }
